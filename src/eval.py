...
def main():
    ...
    for key, path in [("logreg", models_dir/"logreg.joblib"),
        ...
        if key == "xgboost" and xgb_train_classes is not None:
            # raw_pred is in local 0..K-1 space — map back to global 0..14
            ...
            pred = raw_pred
            proba = raw_proba
        pr, rc, f1, sup = precision_recall_fscore_support(y, pred, labels=np.arange(n_classes), zero_division=0)
        cm = confusion_matrix(y, pred, labels=np.arange(n_classes))
        rec = {
            "macro_precision": float(np.mean(pr)), "macro_recall": float(np.mean(rc)),
            "macro_f1": float(np.mean(f1)),
            "micro_f1": float(precision_recall_fscore_support(y, pred, average="micro", zero_division=0)[2]),
            "per_class": {class_names[i]: {"precision": float(pr[i]), "recall": float(rc[i]),
                "f1": float(f1[i]), "support": int(sup[i])} for i in range(n_classes)},
            "confusion_matrix": cm.tolist(), "confusion_labels": class_names,
        }
        if proba is not None and n_classes > 2:
            try:
                rec["roc_auc_ovr"] = float(roc_auc_score(y, proba, multi_class="ovr", average="macro"))
                rec["roc_auc_ovo"] = float(roc_auc_score(y, proba, multi_class="ovo", average="macro"))
            except Exception:
                rec["roc_auc_ovr"] = rec["roc_auc_ovo"] = None
            try:
                rec["pr_auc_macro"] = float(np.mean([average_precision_score((y==i).astype(int), proba[:,i])
                    for i in range(n_classes) if (y==i).sum() > 0]))
            except Exception:
                rec["pr_auc_macro"] = None
        results[key] = rec
        present = sorted(set(y.tolist()))
        report = classification_report(y, pred, labels=present,
        ...
        pd.DataFrame(cm, index=class_names, columns=class_names).to_csv(out_dir / f"confusion_matrix_{key}.csv")
        pred_labels = le.inverse_transform(pred)
        pd.DataFrame({"true_label": y_raw.values, "pred_label": pred_labels, "model": key}).to_csv(
            out_dir / f"predictions_{key}.csv", index=False)
        base = model
        if hasattr(model, "steps"):
            for _, step in model.steps:
                if hasattr(step, "feature_importances_"): base = step; break
        if hasattr(base, "feature_importances_"):
            pd.DataFrame({"feature": feat, "importance": base.feature_importances_}).sort_values(
                "importance", ascending=False).to_csv(out_dir / f"feature_importance_{key}.csv", index=False)
    ...
    err_pairs = sorted([(class_names[i], class_names[j], int(cm_best[i,j]))
        for i in range(cm_best.shape[0]) for j in range(cm_best.shape[1])
        if i != j and cm_best[i,j] > 0], key=lambda x: -x[2])
    summary = {k: {kk: vv for kk, vv in v.items()
        if kk not in ("per_class","confusion_matrix","confusion_labels")} for k, v in results.items()}
